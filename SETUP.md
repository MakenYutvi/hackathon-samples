...
6. Финальная проверка — выведи единый чек-лист со статусом и версиями (если `python` не работает, используй `py`):
   git --version; python --version (или py --version); rg --version; node --version; gh --version; gh auth status;
